...
<div class="row">
  ...
  <!-- 🔍 Cari Barang + Hasil Pencarian dalam satu card -->
  <div class="col-sm-12">
    <div class="card card-primary mb-3">
      ...
      <div class="card-body">
        <input type="text" id="cari" class="form-control mb-3" placeholder="Masukan Kode / Nama Barang...">
    ...
    <div class="card card-primary">
      <div class="card-header bg-primary text-white">
        <h5><i class="fa fa-shopping-cart"></i> KASIR
          <a class="btn btn-danger float-right"
            ...
            href="fungsi/hapus/hapus.php?penjualan=jual&csrf_token=<?php echo urlencode(csrf_get_token());?>">
            <b>RESET KERANJANG</b>
          </a>
        </h5>
      </div>
      <div class="card-body">
        <div id="keranjang" class="table-responsive">
          <table class="table table-bordered">
            <tr>
              ...
              <td><input type="text" readonly="readonly" class="form-control"
                  value="<?= htmlspecialchars(date('j F Y, G:i'), ENT_QUOTES, 'UTF-8');?>" name="tgl"></td>
            </tr>
          </table>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ...
                  <a href="fungsi/hapus/hapus.php?jual=jual&id=<?= htmlspecialchars($isi['id_penjualan'], ENT_QUOTES, 'UTF-8');?>&brg=<?= htmlspecialchars($isi['id_barang'], ENT_QUOTES, 'UTF-8');?>&jml=<?= urlencode($isi['jumlah']); ?>&csrf_token=<?php echo urlencode(csrf_get_token());?>" class="btn btn-danger"><i class="fa fa-times"></i></a>
                </td>
              </tr>
          ...
          <?php
          $hasil = $lihat->jumlah();
          $total = 0.0; $bayar = 0.0; $hitung = 0.0;
          if($isNotaYes && $_SERVER['REQUEST_METHOD'] === 'POST') {
              ...
              if($totalInput !== null && $totalInput !== false && $totalInput !== '') $total = (float)$totalInput;
              if($bayarInput !== null && $bayarInput !== false && $bayarInput !== '') $bayar = (float)$bayarInput;
              ...
              if($bayar > 0.0) {
                  ...
                  if($bayar >= $total) {
                      ...
                      for($x = 0; $x < $jumlahDipilih; $x++) {
                          ...
                          $memberId = (int)($idMemberList[$x] ?? 0);
                          $jumlahItem = (int)($jumlahList[$x] ?? 0);
                          $totalItem = (float)($totalList[$x] ?? 0.0);
                          $tglInputItem = trim((string)($tglInputList[$x] ?? ''));
                          $periodeItem = trim((string)($periodeList[$x] ?? ''));

                          if($barangId === '' || $memberId <= 0 || $jumlahItem <= 0 || $tglInputItem === '' || $periodeItem === '') continue;
            ...
            <table class="table table-stripped">
              <tr>
                ...
                <td><button class="btn btn-success"><i class="fa fa-shopping-cart"></i> Bayar</button>
                  <?php if($isNotaYes){?>
                  <a class="btn btn-danger" href="fungsi/hapus/hapus.php?penjualan=jual&csrf_token=<?php echo urlencode(csrf_get_token());?>"><b>RESET</b></a>
                  <?php }?>
                </td>
              </tr>
            </table>
          ...
          <table class="table table-stripped">
            <tr>
              ...
              <td></td>
              <td>
                <a href="print.php?nm_member=<?php echo urlencode($_SESSION['admin']['nm_member']);?>&bayar=<?php echo urlencode($bayar);?>&kembali=<?php echo urlencode($hitung);?>" target="_blank">
                  <button class="btn btn-secondary"><i class="fa fa-print"></i> Print Bukti Pembayaran</button>
                </a>
              </td>
            </tr>
          </table>
    ...
    timer = setTimeout(function(){
      if(keyword.length > 0){
        $.ajax({
          ...
          data:{ keyword: keyword, csrf_token: window.csrfToken || '' },
          beforeSend: function(){
            $("#hasil_cari").hide();
            $("#tunggu").html('<p style="color:green">Mencari...</p>');
          },
